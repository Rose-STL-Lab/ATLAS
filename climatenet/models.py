...
class CGNetModule(nn.Module):
    ...
    def __init__(self, classes=19, channels=4, M=3, N= 21, dropout_flag = False):
        ...
        self.sample1 = InputInjection(1)  #down-sample for Input Injection, factor=2 (1/4)
        self.sample2 = InputInjection(2)  #down-sample for Input Injection, factor=4 (1/8)
...
class CGNetModule2(nn.Module):
    """
    CGNet (Wu et al, 2018: https://arxiv.org/pdf/1811.08201.pdf) implementation.
    This is taken from their implementation, we do not claim credit for this.
    """
    def __init__(self, classes=19, channels=4, M=3, N= 21, dropout_flag = False, freeze=False):
        """
        args:
          classes: number of classes in the dataset. Default is 19 for the cityscapes
          M: the number of blocks in stage 2
        ...
        """
        super().__init__()
        self.level1_0 = ConvBNPReLU(channels, 32, 3, 2, freeze)      # feature map size divided 2, 1/2

        conv1 = nn.Conv2d(32, 32, (3, 3), stride=1, bias=False)
        self.level1_1 = SharedConvBNPReLU(32, 32, 3, 1, conv1)                      
        self.level1_2 = SharedConvBNPReLU(32, 32, 3, 1, conv1)      
        ...
        #stage 2
        self.level2_0 = ContextGuidedBlock_Down(32 + channels, 64, dilation_rate=2, reduction=8, freeze=freeze)  
        self.level2 = nn.ModuleList()
        conv1x1_2 = ConvBNPReLU(64, 32, 1, 1, freeze)  #1x1 Conv is employed to reduce the computation
        F_loc_2 = ChannelWiseConv(32, 32, 3, 1, freeze) # local feature
        F_sur_2 = ChannelWiseDilatedConv(32, 32, 3, 1, 2, freeze) # surrounding context
        for i in range(0, M-1):
            self.level2.append(SharedContextGuidedBlock(64, 64, dilation_rate=2, reduction=8, conv=conv1x1_2, f_loc=F_loc_2, f_sur=F_sur_2))  #CG block
        self.bn_prelu_2 = BNPReLU(128 + channels)
        
        #stage 3
        self.level3_0 = ContextGuidedBlock_Down(128 + channels, 128, dilation_rate=4, reduction=16, freeze=freeze) 
        self.level3 = nn.ModuleList()
        conv1x1_3 = ConvBNPReLU(128, 64, 1, 1, freeze)  #1x1 Conv is employed to reduce the computation
        F_loc_3 = ChannelWiseConv(64, 64, 3, 1, freeze) # local feature
        F_sur_3 = ChannelWiseDilatedConv(64, 64, 3, 1, 4, freeze) # surrounding context
        for i in range(0, N-1):
            self.level3.append(SharedContextGuidedBlock(128 , 128, dilation_rate=4, reduction=16, conv=conv1x1_3, f_loc=F_loc_3, f_sur=F_sur_3)) # CG block
        self.bn_prelu_3 = BNPReLU(256)
        ...
        if dropout_flag:
            ...
            self.classifier = nn.Sequential(Conv(256, classes, 1, 1, freeze))
