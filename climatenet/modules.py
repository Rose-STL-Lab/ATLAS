...
from climatenet.utils.utils import freeze_grad
...
class SharedConvBNPReLU(nn.Module):
    def __init__(self, nIn, nOut, kSize, stride=1, conv=None):
        """
        args:
...
class ConvBNPReLU(nn.Module):
    def __init__(self, nIn, nOut, kSize, stride=1, freeze=False):
        """
        args:
            nIn: number of input channels
            nOut: number of output channels
...
class ChannelWiseDilatedConv(nn.Module):
    def __init__(self, nIn, nOut, kSize, stride=1, d=1, freeze=False):
        """
        args:
           nIn: number of input channels
           nOut: number of output channels, default (nIn == nOut)
...
class ContextGuidedBlock_Down(nn.Module):
    ...
    """
    def __init__(self, nIn, nOut, dilation_rate=2, reduction=16, freeze=False):
        """
        args:
        ...
        if freeze:
            freeze_grad(self.conv1x1.conv)
            freeze_grad(self.F_loc.conv)
            freeze_grad(self.F_sur.conv)
        self.bn = nn.BatchNorm2d(2*nOut, eps=1e-3)
        self.act = nn.PReLU(2*nOut)
        self.reduce = Conv(2*nOut, nOut,1,1, freeze)  #reduce dimension: 2*nOut--->nOut
...
class SharedContextGuidedBlock(nn.Module):
    def __init__(self, nIn, nOut, dilation_rate=2, reduction=16, add=True, conv=None, f_loc=None, f_sur=None):
        """
        args:
           nIn: number of input channels
           nOut: number of output channels, 
        ...
        super().__init__()
        n= int(nOut/2)
        self.conv1x1 = conv #1x1 Conv is employed to reduce the computation
        self.F_loc = f_loc # local feature
        self.F_sur = f_sur # surrounding context
        freeze_grad(self.conv1x1.conv)
        freeze_grad(self.F_loc.conv)
        freeze_grad(self.F_sur.conv)
